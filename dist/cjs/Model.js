...
class Model {
    ...
    prepModel(schemaFields, block, prefix = '') {
        ...
        for (let [name, field] of Object.entries(schemaFields)) {
            ...
            if (!field.type) {
                throw new Error_js_1.OneTableArgError(`Missing field type for ${pathname}`);
            }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (err.message.indexOf('ConditionalCheckFailed') >= 0) {
                    ...
                    throw new Error_js_1.OneTableError(`Cannot create unqiue attributes "${names}" for "${this.name}", ` +
    ...
    */
    removeUnique(properties, params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let field of fields) {
                if (!properties[field.name]) {
                    throw new Error_js_1.OneTableArgError(`Cannot remove unique field "${field.name}" for model "${this.name}", must provide "${field.name}" value`, { properties });
                }
                let pk = `_unique#${this.name}#${field.attribute}#${properties[field.name]}`;
                let sk = `_unique#`;
            ...
            if (transactHere) {
                yield this.table.transact('write', transaction, params);
            }
        });
    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            */
            try {
                ...
                if (err.message.indexOf('ConditionalCheckFailed') >= 0) {
                    let names = fields.map(f => f.name).join(', ');
                    throw new Error_js_1.OneTableError(`Cannot update unqiue attributes "${names}" for "${this.name}", ` +
                        `an item of the same name already exists.`, { properties, transaction, code: 'UniqueError' });
                }
                throw err;
            }
            ...
            if (block == this.block) {
                ...
                if (properties[name] == null && attribute == index.sort && params.high && KeysOnly[op]) {
                    if (op == 'delete' && !params.many) {
                        throw new Error_js_1.OneTableError('Missing sort key', { code: 'MissingError' });
                    }
                    /*
