    ...
    addUpdates() {
        ...
        if (params.add) {
            ...
            for (let [key, value] of Object.entries(params.add)) {
                if (key == this.hash || key == this.sort) {
                    throw new Error_js_1.OneTableArgError('Cannot add to hash or sort');
                }
        ...
        if (params.delete) {
            for (let [key, value] of Object.entries(params.delete)) {
                if (key == this.hash || key == this.sort) {
                    throw new Error_js_1.OneTableArgError('Cannot delete hash or sort');
                }
                this.already[key] = true;
                let target = this.makeTarget(fields, key);
        ...
        if (params.remove) {
            ...
            for (let key of params.remove) {
                if (key == this.hash || key == this.sort) {
                    throw new Error_js_1.OneTableArgError('Cannot remove hash or sort');
                }
                if (fields.required) {
        ...
        if (params.set) {
            for (let [key, value] of Object.entries(params.set)) {
                if (key == this.hash || key == this.sort) {
                    throw new Error_js_1.OneTableArgError('Cannot set hash or sort');
                }
                this.already[key] = true;
                let target = this.makeTarget(fields, key);
